...
TeleOpJoypad::TeleOpJoypad(ros::NodeHandle &nh)
{
    ...
    arm_parameters_available = false;
	...
	if(nh.getParam("/arm_1/arm_controller/joints", param_list))
	{
...
void TeleOpJoypad::publishCommands()
{
    ...
    if(button_print_arm_states_prev_ == false && button_print_arm_states_ == true && arm_parameters_available)
        printArmJointStates();
}
